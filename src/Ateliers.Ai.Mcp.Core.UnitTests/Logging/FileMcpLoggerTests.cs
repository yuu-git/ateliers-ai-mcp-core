...
public class FileMcpLoggerTests : IDisposable
{
    ...
    [Fact]
    [Trait("説明", @"ログファイル名に日付フォーマットであること")]
    ...
    [Fact]
    [Trait("説明", @"存在しないログディレクトリで検索した場合、空のセッションを返すこと")]
    ...
    [Fact]
    [Trait("説明", @"ログディレクトリが指定されていない場合、デフォルトディレクトリを使用すること")]
    ...
    [Fact]
    [Trait("説明", @"相関IDでログセッションを正しく読み取ること")]
